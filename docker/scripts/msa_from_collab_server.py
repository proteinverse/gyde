...
def parse_fasta(text_stream):
    ...
    for line in text_stream:
        ...
        raise Exception("Stream contains no sequences")
    data = []
    for line in text_stream:
        if line.startswith('>'):
            if not data:
                raise Exception(f"Sequence {header} contains no data")
            yield header, ''.join(data)
            header = line[1:].strip()
            data = []
        else:
...
if __name__ == '__main__':
    with open(sys.argv[1], 'rt') as in_file:
        for (header, seq) in parse_fasta(in_file):
            lines = run_mmseqs2(seq)
            with open('aligned.a3m', 'wt') as out:
                out.write(lines[0])
